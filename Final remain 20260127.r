...
# ========== T2MI: MORTALITY COMPARISON WITH vs WITHOUT TRIGGER ==========

cat("\n\n=== T2MI MORTALITY: WITH TRIGGER vs WITHOUT TRIGGER ===\n")

# Filter to T2MI patients only
t2mi_mortality <- obs12_with_pmi %>%
  ...
  mutate(
    T2MI_Group = if_else(PMI_category == "T2MI_with_cause", "T2MI with trigger", "T2MI without trigger")
  )

cat("\n--- In-Hospital Mortality in T2MI Patients ---\n")
print(t2mi_mortality %>% select(T2MI_Group, N, Deaths, Mortality_pct), n = Inf)

# Statistical test: T2MI with trigger vs without trigger
cat("\n--- Statistical Test: T2MI with trigger vs without trigger ---\n")

t2mi_test_data <- obs12_with_pmi %>%
  filter(PMI_category %in% c("T2MI_with_cause", "T2MI_without_cause")) %>%
  mutate(
    T2MI_Group = factor(PMI_category, 
                        levels = c("T2MI_without_cause", "T2MI_with_cause"),
                        labels = c("T2MI without trigger", "T2MI with trigger"))
  ) %>%
  select(T2MI_Group, death_in_hospital)

if(nrow(t2mi_test_data) > 0) {
  t2mi_table <- table(t2mi_test_data$T2MI_Group, t2mi_test_data$death_in_hospital)
  ...
  if(min(t2mi_table) < 5) {
    cat("Using Fisher's exact test (cell count < 5)\n")
    t2mi_test <- fisher.test(t2mi_table)
    cat("Fisher's exact test p-value:", format.pval(t2mi_test$p.value, digits = 3), "\n")
    p_val_t2mi <- t2mi_test$p.value
  } else {
    cat("Using Chi-square test\n")
    t2mi_test <- chisq.test(t2mi_table)
    print(t2mi_test)
    p_val_t2mi <- t2mi_test$p.value
  }
  
  cat("\n*** P-VALUE for T2MI mortality (with trigger vs without trigger): ", 
      format.pval(p_val_t2mi, digits = 3), " ***\n", sep = "")
  
  if(p_val_t2mi < 0.05) {
    cat("INTERPRETATION: Significant difference in mortality between T2MI with and without trigger\n\n")
  } else {
    cat("INTERPRETATION: No significant difference in mortality between T2MI with and without trigger\n\n")
  }
} else {
  cat("Insufficient T2MI patients for statistical testing\n\n")
}
...
cat("✓ **NEW: T2MI mortality comparison with p-value (with trigger vs without trigger)**\n")
