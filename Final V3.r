...
# Load gridExtra for potential tabular layouts (patchwork handles plot combining)
...
pie_cardiac <- obs12_with_pmi %>%
  filter(Cause_cardiac_yes == 1) %>%
  mutate(cause_label = case_when(
    cause_cardiac == "STEMI"         ~ "STEMI",
    cause_cardiac == "NSTEMI"        ~ "NSTEMI",
    cause_cardiac == "UA"            ~ "Unstable angina",
    cause_cardiac == "HF"            ~ "Heart failure",
    cause_cardiac == "TS"            ~ "Takotsubo",
    cause_cardiac == "cardiac_other" ~ "Other",
    TRUE                             ~ as.character(cause_cardiac)
  )) %>%
  count(cause_label, sort = TRUE) %>%
  mutate(
    pct   = n / sum(n),
...
cif_30d_plot_data <- data.frame(
  time = numeric(), est = numeric(), group = character(), event = character()
)
for (nm in names(cif_30d)) {
  ...
  cif_30d_plot_data <- rbind(cif_30d_plot_data, data.frame(
    time = cif_30d[[nm]]$time, est = cif_30d[[nm]]$est,
    group = grp, event = evt
  ))
}
...
max_cif_30d   <- max(pa_data_30d$est * 100, na.rm = TRUE)
y_max_30d     <- max(ceiling(max_cif_30d / 5) * 5, 5)
y_breaks_30d  <- seq(0, y_max_30d, ifelse(y_max_30d <= 20, 5, 10))

panel_a_30d <- ggplot(pa_data_30d, aes(x = time, y = est * 100, color = group)) +
  geom_step(linewidth = 1) +
  scale_color_manual(values = c("Cardiac" = "#E64B35", "Extracardiac" = "#4DBBD5")) +
  scale_x_continuous(limits = c(0, 30), breaks = seq(0, 30, 5)) +
  scale_y_continuous(limits = c(0, y_max_30d), breaks = y_breaks_30d) +
...
panel_b_30d <- ggplot(pb_data_30d, aes(x = time, y = est * 100, color = group)) +
  geom_step(linewidth = 1) +
  scale_color_manual(values = c("Cardiac" = "#E64B35", "Extracardiac" = "#4DBBD5")) +
  scale_x_continuous(limits = c(0, 30), breaks = seq(0, 30, 5)) +
  scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, 20)) +
