...
# Extract VAS pain scores from postoperative vitals
vas_data <- postoperativevitals %>%
  filter(code_display_original == "VAS") %>%
  select(pseudonym_value, valueQuantity_value, effectiveDateTime) %>%
  mutate(
...
cat("\nTotal VAS measurements:", nrow(vas_data), "\n")
cat("Unique patients with VAS measurements:", length(unique(vas_data$pseudonym_value)), "\n")

# Link VAS data to OBS12 patients with PMI classifications
# IMPORTANT: Using HIGHEST (worst) VAS score per patient for classification
  ...
  left_join(
    vas_data %>% 
      ...
      summarise(
        ...
        # Determine worst VAS category per patient based on HIGHEST VAS score
        VAS_category = case_when(
...
cat("\nOBS12 patients with VAS measurements:", sum(!is.na(vas_obs12$VAS_category)), "\n")
cat("OBS12 patients without VAS measurements:", sum(is.na(vas_obs12$VAS_category)), "\n")
cat("\n*** NOTE: Each patient classified by their HIGHEST (worst) VAS score measured ***\n")
...
vas_mortality_plot <- ggplot(vas_plot_data, 
  ...
  labs(
    ...
    subtitle = paste0("Stratified by PMI Type (OBS12)\n", p_text_mortality),
...
vas_distribution_plot <- ggplot(vas_by_pmi, 
  ...
  labs(
    ...
    subtitle = paste0("OBS12 Patients\n", p_text_distribution),
    x = "VAS Pain Score Category",
    y = "Percentage (%)",
    fill = "PMI Type"
  ) +
...
cat("✓ **NEW: In-hospital mortality adjusted for emergency surgery status**\n")
cat("✓ **NEW: Logistic regression models comparing unadjusted vs adjusted mortality**\n")
cat("✓ VAS pain score analysis with distribution and mortality by PMI type\n")
